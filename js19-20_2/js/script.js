...
var el = JSON.stringify(myArr);

// Skills

var skills = _.map(myArr, 'skills');
//console.log(skills);
...
_.forEach(skills, function (key, value) {
    ...
    allSkills = _.concat(allSkills, skills[value]);
   
})

console.log('Все skills', allSkills);

var uniqSkills = _.uniq(allSkills);
console.log('Все skills без повторений', uniqSkills);

var lower = [];

for (var i = 0; i < uniqSkills.length; i++) {
    lower[i] = uniqSkills[i].toLowerCase();    
}
console.log('Все skills без повторений с маленькой буквы', lower);

var sortSkills = _.sortBy(lower);
console.log('Все skills без повторений, отсортированы по алфавиту', sortSkills);

// сортировка name по friends

var namePeople = _.map(myArr, 'name');
console.log('Все name', namePeople);

var  sortName = _.sortBy(myArr, ['friends', 'name']);
var sumfr = [];
_.forEach(sortName, function (key, value) {
   
    sumfr = _.concat(sumfr, sortName[value]);
   
})
//console.log(sortName);

var arrNameFriends = _.map(sortName, 'name');
console.log('Name отсортированы в зависимости от количества друзей (friends)', arrNameFriends);

// All friends

var allFriends = _.map(myArr, 'friends');
//console.log(allFriends);

var friends = [];

_.forEach(allFriends, function (key, value) {
   
    friends = _.concat(friends, allFriends[value]);
   
})

//console.log(friends);

var myFriends = _.map(friends, 'name');
console.log('Все друзья', myFriends);

var uniqFriends = _.uniq(myFriends);
console.log('Все друзья без повторений', uniqFriends);